  ...
  : QPainter(device), mModes(pmDefault), mIsAntialiasing(false)
{
  ...
  QT_VERSION_CHECK( \
    5,              \
    0,              \
    0)  // before Qt5, default pens used to be cosmetic if NonCosmeticDefaultPen
                                        ...
                                        int             totalSize) const
{
  ...
  while (!unfinishedSections.isEmpty() &&
         outerIterations <
           sectionCount * 2)  // the iteration check ist just a failsafe in case
    ...
    if (foundMinimumViolation)
    {
      ...
      for (int i = 0; i < sectionCount; ++i)
      {
        if (!minimumLockedSections.contains(
              i))  // only put sections that haven't hit their minimum back into
                   // the pool
          unfinishedSections.append(i);
        else
          freeSize -=
...
QSize QCPLayoutGrid::minimumSizeHint() const
{
  ...
  result.rheight() +=
    qMax(0, rowCount() - 1) * mRowSpacing + mMargins.top() + mMargins.bottom();
  return result;
}
  ...
  , mCachedMargin(0)
{
  ...
  setLayer(
    mParentPlot
      ->currentLayer());  // it's actually on that layer already, but we want it
                          // in front of the grid, so we place it on there again
    ...
    if (mAutoTickStep)
    {
      ...
      mTickStep =
        ...
        (double)(mAutoTickCount +
                 1e-10);  // mAutoTickCount ticks on average, the small addition
                          // is to prevent jitter on exact integers
...
void QCPAxisPainterPrivate::draw(QCPPainter* painter)
{
  ...
  if (!tickPositions.isEmpty())
  {
    ...
    int tickDir = (type == QCPAxis::atBottom || type == QCPAxis::atRight)
                    ? -1
                    : 1;  // direction of ticks ("inward" is right for left axis
                          // and left for right axis)
    if (QCPAxis::orientation(type) == Qt::Horizontal)
    {
  ...
  const QString& text) const
{
  ...
  result.totalBounds.moveTopLeft(
    QPoint(0, 0));  // want bounding box aligned top left at origin, independent
  ...
  */
  ...
  bool flip =
    qFuzzyCompare(qAbs(tickLabelRotation),
                  90.0);  // perfect +/-90 degree flip. Indicates vertical label
                          // centering on vertical axes.
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale)
{
  ...
  result.fill(
    mBackgroundBrush.style() == Qt::SolidPattern
      ? mBackgroundBrush.color()
      : Qt::transparent);  // if using non-solid pattern, make transparent now
                           // and draw brush pattern later
  QCPPainter painter;
                                ...
                                bool            logarithmic)
{
  ...
  if (!logarithmic)
  {
    ...
    if (mPeriodic)
    {
      ...
      {
        int index =
          (int)((data[dataIndexFactor * i] - range.lower) * posToIndexFactor) %
          mLevelCount;
        if (index < 0)
          index += mLevelCount;
...
void QCPAxisRect::setupFullAxesBox(bool connectRanges)
{
  ...
  {
    connect(
      xAxis, SIGNAL(rangeChanged(QCPRange)), xAxis2, SLOT(setRange(QCPRange)));
    connect(
      yAxis, SIGNAL(rangeChanged(QCPRange)), yAxis2, SLOT(setRange(QCPRange)));
  }
}
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type)
{
  ...
  bool isFirstVisible =
    !axesList.first()
       ->visible();  // if the first axis is visible, the second axis (which is
                     // where the loop starts) isn't the first visible axis, so
...
void QCPPlottableLegendItem::draw(QCPPainter* painter)
{
  ...
  int    textHeight =
    qMax(textRect.height(),
         iconSize.height());  // if text has smaller height than icon, center
                              // text vertically in icon height, else align tops
  painter->drawText(
      ...
      mSelectableParts.testFlag(spLegendBox))
  {
    ...
    setSelectedParts(
      additive ? mSelectedParts ^ spLegendBox
               : mSelectedParts |
                   spLegendBox);  // no need to unset spItems in !additive case,
                                  // because they will be deselected by
                                  // QCustomPlot (they're normal QCPLayerables
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  if (mType != type)
  {
    ...
    mColorAxis.data()->setScaleLogBase(
      logBaseTransfer);  // scaleType is synchronized among axes in realtime via
                         // signals (connected in QCPColorScale ctor), so we
                         // only need to take care of log base here
  ...
  if (mainPen().style() != Qt::NoPen && mainPen().color().alpha() != 0)
  {
    ...
    */
    ...
    else
    {
      ...
      painter->drawPolyline(
        lineData->constData() + segmentStart,
        lineDataSize - segmentStart);  // lineDataSize, because we do want to
                              ...
                              // least two points per pixel on average
  {
    if (lineData)
    {
      ...
      int reversedRound =
        keyAxis->rangeReversed() != (keyAxis->orientation() == Qt::Vertical)
          ? 1
          : 0;  // is used to switch between floor (normal) and ceil (reversed)
                // rounding of currentIntervalStartKey
        ...
        else  // new pixel interval started
        {
                                       ...
                                       // consolidate them to a cluster
          {
            ...
            if (it.key() >
                currentIntervalStartKey +
                  keyEpsilon *
                    2)  // new pixel started further away from previous cluster,
                        // so make sure the last point of the cluster is at a
                        // real data point
              lineData->append(
                         ...
                         const QCPData& data) const
{
  ...
  double skipSymbolMargin =
    mScatterStyle
      .size();  // pixels left blank per side, when mErrorBarSkipSymbol is true
                                    ...
                                    QCPDataMap::const_iterator& upper) const
{
  ...
  bool lowoutlier =
    lbound !=
    mData
      ->constBegin();  // indicates whether there exist points below axis range
  bool highoutlier =
    ubound !=
           ...
           sdNegative)  // range may only be in the negative sign domain
  {
    ...
    while (it != mData->constEnd())
    {
      ...
      if (includeErrors)  // in case point is in valid sign domain but errobars
                          // stretch beyond it, we still want to geht that
                          // point.
      {
  ...
  while (it != mData->constEnd())
  {
                     ...
                     // points or original points if entering R
    {
          ...
          5)  // segment doesn't end in R, so it's a candidate for removal
      {
        ...
        else  // doesn't cross R, line is just moving around in outside regions,
              // so only need to add optimized point(s) at the boundary
              // corner(s)
        {
          *lineData << getOptimizedCornerPoints(prevRegion,
                                                currentRegion,
  ...
  switch (prevRegion)
  {
    case 1:
    {
      switch (currentRegion)
      {
        ...
        case 9:
        {  // in this case we need another distinction of cases: segment may
...
double QCPBarsGroup::keyPixelOffset(const QCPBars* bars, double keyCoord)
{
  ...
  if (index >= 0)
  {
    ...
    if (baseBars.size() % 2 == 1 &&
        index ==
          (baseBars.size() - 1) / 2)  // is center bar (int division on purpose)
    {
      return result;
    }
    else if (index <
...
#ifdef __EXCEPTIONS
      try
      {  // 2D arrays get memory intensive fast. So if the allocation fails, at
    ...
    for (int line = 0; line < lineCount; ++line)
    {
      QRgb* pixels = reinterpret_cast<QRgb*>(mMapImage.scanLine(
        ...
        line));  // invert scanline index because QImage counts scanlines from
                 // top, but our vertical index counts from bottom (mathematical
                 // coordinate system)
                                                   ...
                                                   double timeBinOffset)
{
  ...
  for (int i = 0; i < count; ++i)
  {
    ...
    else  // data point not anymore in current bin, set close of old and open of
          // new bin, and add old to map:
    {
  ...
  else if (qFuzzyIsNull(vec.y()))  // line is horizontal
  {
    ...
    if (gamma >= 0 && gamma <= rect.height())
      result.setLine(
        ...
        rect.right(),
        by +
          gamma);  // no need to check right because we know line is horizontal
  }
  else  // line is skewed
  {
                                       ...
                                       bool  flipVert)
{
  ...
  if (mScaled)
  {
    ...
    {
      mScaledPixmap =
        mPixmap.scaled(finalRect.size(), mAspectRatioMode, mTransformationMode);
                                  ...
                                  QVariant*      details) const
{
  ...
  return qSqrt(distSqrToLine(
    (centerVec - widthVec).toPointF(), (centerVec + widthVec).toPointF(), pos));
}
